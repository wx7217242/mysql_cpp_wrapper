...
public:
    ...
    virtual bool GetBoolean(uint32_t idx, bool def) const = 0;
    
    virtual double GetDouble(uint32_t idx, double def) const = 0;
    
    virtual int32_t GetInt(uint32_t idx, int def) const = 0;
    
    virtual uint32_t GetUInt(uint32_t idx, uint32_t def) const = 0;
    
    virtual int64_t GetInt64(uint32_t idx, int64_t def) const = 0;
    
    virtual uint64_t GetUInt64(uint32_t idx, uint64_t def) const = 0;
    
    virtual uint32_t GetString(uint32_t idx, char* buffer, uint32_t max_buf_len)  const = 0;
    
    virtual bool IsNull(uint32_t idx) const = 0;
